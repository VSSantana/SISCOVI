...
BEGIN
  ...
  FOR i IN 1 .. F_RETORNA_NUMERO_DE_MESES(pInicioPeriodoAquisitivo, pFimPeriodoAquisitivo) LOOP
    ...
    IF (F_EXISTE_DUPLA_CONVENCAO(vCodCargoContrato, vMes, vAno) = FALSE AND F_EXISTE_MUDANCA_PERCENTUAL(vCodContrato, vMes, vAno) = FALSE) THEN
    
      --Define a remuneração do cargo e os percentuais de férias e incidência.
            
      vRemuneracao := F_RETORNA_REMUNERACAO_PERIODO(vCodCargoContrato, vMes, vAno, 1);
      vPercentualFerias := F_RETORNA_PERCENTUAL_PERIODO(vCodContrato, 'Férias', vMes, vAno, 1);
      vPercentualIncidencia := F_RETORNA_PERCENTUAL_PERIODO(vCodContrato, 'Incidência do submódulo 4.1', vMes, vAno, 1);
    ...
    IF (F_EXISTE_DUPLA_CONVENCAO(vCodCargoContrato, vMes, vAno) = TRUE AND F_EXISTE_MUDANCA_PERCENTUAL(vCodContrato, vMes, vAno) = FALSE) THEN
    
      --Define a remuneração do cargo para a primeira metade do mês e os percentuais do mês.
            
      vRemuneracao := F_RETORNA_REMUNERACAO_PERIODO(vCodCargoContrato, vMes, vAno, 2);
      vPercentualFerias := F_RETORNA_PERCENTUAL_PERIODO(vCodContrato, 'Férias', vMes, vAno, 1);
      vPercentualIncidencia := F_RETORNA_PERCENTUAL_PERIODO(vCodContrato, 'Incidência do submódulo 4.1', vMes, vAno, 1);
    ...
    IF (F_EXISTE_DUPLA_CONVENCAO(vCodCargoContrato, vMes, vAno) = FALSE AND F_EXISTE_MUDANCA_PERCENTUAL(vCodContrato, vMes, vAno) = TRUE) THEN
    
      --Define a remuneração do cargo no mês e os percentuais do mês da primeira metade do mês.
            
      vRemuneracao := F_RETORNA_REMUNERACAO_PERIODO(vCodCargoContrato, vMes, vAno, 1);
      vPercentualFerias := F_RETORNA_PERCENTUAL_PERIODO(vCodContrato, 'Férias', vMes, vAno, 2);
      vPercentualIncidencia := F_RETORNA_PERCENTUAL_PERIODO(vCodContrato, 'Incidência do submódulo 4.1', vMes, vAno, 2);
    ...
    IF (F_EXISTE_DUPLA_CONVENCAO(vCodCargoContrato, vMes, vAno) = TRUE AND F_EXISTE_MUDANCA_PERCENTUAL(vCodContrato, vMes, vAno) = TRUE) THEN
