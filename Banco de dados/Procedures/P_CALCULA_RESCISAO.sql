...
BEGIN
    ...
    FOR i IN 1 .. vNumeroDeMeses LOOP
      ...
      IF (vMes = 12 OR (vMes = EXTRACT(month FROM pDataDesligamento) AND vAno = EXTRACT(year FROM pDataDesligamento))) THEN
        ...
        ELSE
          ...
          vDecimoTerceiro :=  vDecimoTerceiro + vTotalDecimoTerceiro;
          vTotalDecimoTerceiro := 0;
        ...
        ELSE
          ...
          vIncidSubmod41DecTer :=  vIncidSubmod41DecTer + vTotalIncidenciaDecimoTerceiro;
          vTotalIncidenciaDecimoTerceiro := 0;
  ...
  BEGIN
    ...
    FOR i IN 1 .. vNumeroDeMeses LOOP
      ...
      IF (vMes = 12 OR (vMes = EXTRACT(month FROM pDataDesligamento) AND vAno = EXTRACT(year FROM pDataDesligamento))) THEN
        ...
        --Para o valor de férias.

        IF (vTotalFerias - F_SALDO_CONTA_VINCULADA (pCodCargoFuncionario, vAno, 2, 1) <= 0) THEN
        ...
        --Para o valor do terço constituicional.
        
        IF (vTotalTercoConstitucional - F_SALDO_CONTA_VINCULADA (pCodCargoFuncionario, vAno, 2, 2) <= 0) THEN
        ...
        --Para o valor de incidência de férias.
        
        IF (vTotalIncidenciaFerias - F_SALDO_CONTA_VINCULADA (pCodCargoFuncionario, vAno, 2, 101) <= 0) THEN

          vTotalIncidenciaFerias := 0;

        ELSE

          vIncidSubmod41Ferias :=  vIncidSubmod41Ferias + vTotalIncidenciaFerias;
          vTotalIncidenciaFerias := 0;

        END IF;

        --Para o valor de incidência do terço constitucional.
        
        IF (vTotalIncidenciaTerco - F_SALDO_CONTA_VINCULADA (pCodCargoFuncionario, vAno, 2, 102) <= 0) THEN

          vTotalIncidenciaTerco := 0;

        ELSE

          vIncidSubmod41Terco :=  vIncidSubmod41Terco + vTotalIncidenciaTerco;
          vTotalIncidenciaTerco := 0;
  ...
  INSERT INTO tb_restituicao_rescisao (cod_cargo_funcionario,
