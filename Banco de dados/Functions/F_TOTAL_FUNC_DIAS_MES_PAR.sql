create or replace function "F_TOTAL_FUNC_DIAS_MES_PAR"(pCodCargoContrato NUMBER, pMes NUMBER, pAno NUMBER, pDias NUMBER) RETURN NUMBER
IS

--Retorna o número de funcionários que trabalharam um período inferior a 15 e exatamente pDias.
...
--e a data de desligamento é nula ou maior que o mês de cálculo.
  ...
  SELECT COUNT(DISTINCT(cod))
    ...
    WHERE cod_cargo_contrato = pCodCargoContrato
      ...
      AND (LAST_DAY(TRUNC(TO_DATE('01/' || TO_CHAR(pMes) || '/' || TO_CHAR(pAno)))) - TRUNC(data_disponibilizacao) + 1) = pDias;
...
--Caso com data de desligamento no mês de cálculo e data de disponibilização inferior ao mês de cálculo.
  ...
  SELECT COUNT(DISTINCT(cod))
    ...
    WHERE cod_cargo_contrato = pCodCargoContrato
      ...
      AND (TRUNC(data_desligamento) - TRUNC(TO_DATE('01/' || TO_CHAR(pMes) || '/' || TO_CHAR(pAno))) + 1) = pDias;
...
--e a data de desligamento também está no mesmo mês.
  ...
  SELECT COUNT(DISTINCT(cod))
    ...
    WHERE cod_cargo_contrato = pCodCargoContrato
      ...
      AND (TRUNC(data_desligamento) - TRUNC(data_disponibilizacao) + 1) = pDias;
